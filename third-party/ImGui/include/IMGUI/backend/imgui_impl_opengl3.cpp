...
static ImGui_ImplOpenGL3_Data* ImGui_ImplOpenGL3_GetBackendData()
{
	return ImGui::GetCurrentContext()
		   ? static_cast<ImGui_ImplOpenGL3_Data*>(ImGui::GetIO().
...
bool ImGui_ImplOpenGL3_Init(const char* glsl_version)
{
	...
	auto gl_version_str = (const char*)glGetString(GL_VERSION);
#if defined(IMGUI_IMPL_OPENGL_ES2)
    // GLES 2
    bd->GlVersion = 200;
	...
	*/
	...
	IM_ASSERT(
			static_cast<int>(strlen(glsl_version)) + 2 < IM_COUNTOF(bd->
				GlslVersionString));
	...
	for (GLint i = 0; i < num_extensions; i++)
	{
		...
		if (extension != nullptr && strcmp(extension, "GL_ARB_clip_control") ==
			0)
											   ...
											   GLuint vertex_array_object)
{
	...
	GL_CALL(
			glVertexAttribPointer(bd->AttribLocationVtxPos, 2, GL_FLOAT,
	...
	GL_CALL(
			glVertexAttribPointer(bd->AttribLocationVtxColor, 4,
				GL_UNSIGNED_BYTE, GL_TRUE, sizeof(ImDrawVert), (GLvoid*)offsetof
				(ImDrawVert, col)));
}
...
void ImGui_ImplOpenGL3_RenderDrawData(ImDrawData* draw_data)
{
	...
	for (const ImDrawList* draw_list : draw_data->CmdLists)
	{
		...
		const GLsizeiptr vtx_buffer_size =
				static_cast<GLsizeiptr>(draw_list->VtxBuffer.Size) * static_cast
				<int>(sizeof(ImDrawVert));
		const GLsizeiptr idx_buffer_size =
				static_cast<GLsizeiptr>(draw_list->IdxBuffer.Size) * static_cast
		...
		if (bd->UseBufferSubData)
		{
			if (bd->VertexBufferSize < vtx_buffer_size)
			{
				...
				GL_CALL(
						glBufferData(GL_ARRAY_BUFFER, bd->VertexBufferSize,
			...
			if (bd->IndexBufferSize < idx_buffer_size)
			{
				...
				GL_CALL(
						glBufferData(GL_ELEMENT_ARRAY_BUFFER, bd->
							IndexBufferSize, nullptr, GL_STREAM_DRAW));
			}
			GL_CALL(
					glBufferSubData(GL_ARRAY_BUFFER, 0, vtx_buffer_size,
						static_cast<const GLvoid*>(draw_list->VtxBuffer.Data)));
			GL_CALL(
					glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, 0, idx_buffer_size,
						static_cast<const GLvoid*>(draw_list->IdxBuffer.Data)));
		}
		else
		{
			GL_CALL(
					glBufferData(GL_ARRAY_BUFFER, vtx_buffer_size, static_cast<
		...
		for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++)
		{
			...
			else
			{
				...
				GL_CALL(
						glScissor(static_cast<int>(clip_min.x), static_cast<int>
							((float)fb_height - clip_max.y), static_cast<int>(
								clip_max.x - clip_min.x), static_cast<int>(
								clip_max.y - clip_min.y)));

				// Bind texture, Draw
				GL_CALL(
						glBindTexture(GL_TEXTURE_2D, static_cast<GLuint>((
							intptr_t)pcmd->GetTexID())));
#ifdef IMGUI_IMPL_OPENGL_MAY_HAVE_VTX_OFFSET
				if (bd->GlVersion >= 320)
					GL_CALL(
						glDrawElementsBaseVertex(GL_TRIANGLES, static_cast<
							GLsizei>(pcmd->ElemCount), sizeof(ImDrawIdx) == 2 ?
							GL_UNSIGNED_SHORT : GL_UNSIGNED_INT, (void*)
							static_cast<intptr_t>(pcmd->IdxOffset * sizeof(
								ImDrawIdx)), static_cast<GLint>(pcmd->VtxOffset)
						));
				else
#endif
					GL_CALL(
						glDrawElements(GL_TRIANGLES, static_cast<GLsizei>(pcmd->
								ElemCount), sizeof(ImDrawIdx) == 2 ?
							GL_UNSIGNED_SHORT : GL_UNSIGNED_INT, (void*)
							static_cast<
...
{
	GLuint gl_tex_id = static_cast<GLuint>((intptr_t)tex->TexID);
...
void ImGui_ImplOpenGL3_UpdateTexture(ImTextureData* tex)
{
	...
	if (tex->Status == ImTextureStatus_WantCreate || tex->Status ==
	...
	if (tex->Status == ImTextureStatus_WantCreate)
	{
		...
		// Store identifiers
		tex->SetTexID(static_cast<ImTextureID>((intptr_t)gl_texture_id));
	...
	else if (tex->Status == ImTextureStatus_WantUpdates)
	{
		...
		for (ImTextureRect& r : tex->Updates)
			GL_CALL(
				glTexSubImage2D(GL_TEXTURE_2D, 0, r.x, r.y, r.w, r.h, GL_RGBA,
	...
	else if (tex->Status == ImTextureStatus_WantDestroy && tex->UnusedFrames >
			 0)
...
bool ImGui_ImplOpenGL3_CreateDeviceObjects()
{
	...
	{
		glGetIntegerv(
				GL_PIXEL_UNPACK_BUFFER_BINDING, &last_pixel_unpack_buffer);
	...
	bd->AttribLocationProjMtx = glGetUniformLocation(
			bd->ShaderHandle, "ProjMtx");
	bd->AttribLocationVtxPos = static_cast<GLuint>(glGetAttribLocation(
			bd->ShaderHandle, "Position"));
	bd->AttribLocationVtxUV = static_cast<GLuint>(glGetAttribLocation(
			bd->ShaderHandle, "UV"));
	bd->AttribLocationVtxColor = static_cast<GLuint>(glGetAttribLocation(
			bd->ShaderHandle, "Color"));
