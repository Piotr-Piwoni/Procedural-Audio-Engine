...
static ImGui_ImplGlfw_Data* ImGui_ImplGlfw_GetBackendData()
{
	...
	return ImGui::GetCurrentContext()
		   ? static_cast<ImGui_ImplGlfw_Data*>(ImGui::GetIO().
...
static ImGui_ImplGlfw_Data* ImGui_ImplGlfw_GetBackendData(GLFWwindow* window)
{
	...
	return static_cast<ImGui_ImplGlfw_Data*>(ImGui::GetIO(ctx).
...
void ImGui_ImplGlfw_InstallCallbacks(GLFWwindow* window)
{
	...
	bd->PrevUserCallbackCursorPos = glfwSetCursorPosCallback(
			window, ImGui_ImplGlfw_CursorPosCallback);
	...
	bd->PrevUserCallbackKey = glfwSetKeyCallback(
			window, ImGui_ImplGlfw_KeyCallback);
	bd->PrevUserCallbackChar = glfwSetCharCallback(
			window, ImGui_ImplGlfw_CharCallback);
...
#ifdef _WIN32
	auto hwnd = static_cast<HWND>(main_viewport->PlatformHandleRaw);
	SetPropA(hwnd, "IMGUI_BACKEND_DATA", bd);
	bd->PrevWndProc = (WNDPROC)GetWindowLongPtrW(hwnd, GWLP_WNDPROC);
	IM_ASSERT(bd->PrevWndProc != nullptr);
	SetWindowLongPtrW(static_cast<HWND>(main_viewport->PlatformHandleRaw),
					  GWLP_WNDPROC, (LONG_PTR)ImGui_ImplGlfw_WndProc);
#endif
...
#ifdef _WIN32
	ImGuiViewport* main_viewport = ImGui::GetMainViewport();
	SetPropA(static_cast<HWND>(main_viewport->PlatformHandleRaw),
			 "IMGUI_BACKEND_DATA", nullptr);
	SetWindowLongPtrW(static_cast<HWND>(main_viewport->PlatformHandleRaw),
					  GWLP_WNDPROC, (LONG_PTR)bd->PrevWndProc);
	...
	for (int n = 0; n < platform_io.Viewports.Size; n++)
	{
		...
		if (is_window_focused)
		{
			...
			if (io.WantSetMousePos)
				glfwSetCursorPos(window, mouse_pos_prev.x - viewport->Pos.x,
			...
			if (bd->MouseWindow == nullptr)
			{
				...
				bd->LastValidMousePos = ImVec2(static_cast<float>(mouse_x),
											   static_cast<float>(mouse_y));
				io.AddMousePosEvent(static_cast<float>(mouse_x),
									static_cast<float>(mouse_y));
			}
		}
...
static void ImGui_ImplGlfw_UpdateMouseCursor()
{
	...
	for (int n = 0; n < platform_io.Viewports.Size; n++)
	{
		auto window = static_cast<GLFWwindow*>(platform_io.Viewports[n]->
			PlatformHandle);
...
static void ImGui_ImplGlfw_UpdateMonitors()
{
	...
	if (monitors_count == 0)
		// Preserve existing monitor list if there are none. Happens on macOS sleeping (#5683)
		return;
	...
	for (int n = 0; n < monitors_count; n++)
	{
		...
		monitor.MainSize = monitor.WorkSize = ImVec2(
								   static_cast<float>(vid_mode->width),
								   static_cast<float>(vid_mode->height));
		...
		{
			monitor.WorkPos = ImVec2(static_cast<float>(x),
									 static_cast<float>(y));
			monitor.WorkSize = ImVec2(static_cast<float>(w),
									  static_cast<float>(h));
		}
#endif
		float scale =
		...
		monitor.PlatformHandle = static_cast<void*>(glfw_monitors[n]);
		// [...] GLFW doc states: "guaranteed to be valid only until the monitor configuration changes"
...
static void ImGui_ImplGlfw_WindowPosCallback(GLFWwindow* window, int, int)
{
	if (ImGuiViewport* viewport = ImGui::FindViewportByPlatformHandle(window))
	{
		if (auto vd = static_cast<ImGui_ImplGlfw_ViewportData*>(viewport->
			PlatformUserData))
		{
			bool ignore_event = (
...
static void ImGui_ImplGlfw_CreateWindow(ImGuiViewport* viewport)
{
...
#endif
	glfwWindowHint(
			GLFW_DECORATED, (viewport->Flags & ImGuiViewportFlags_NoDecoration)
							? false : true);
#if GLFW_HAS_WINDOW_TOPMOST
	glfwWindowHint(GLFW_FLOATING, (viewport->Flags & ImGuiViewportFlags_TopMost)
								  ? true : false);
#endif
	GLFWwindow* share_window = (bd->ClientApi == GlfwClientApi_OpenGL)
							   ? bd->Window : nullptr;
	vd->Window = glfwCreateWindow(static_cast<int>(viewport->Size.x),
	...
	SetPropA(static_cast<HWND>(viewport->PlatformHandleRaw),
			 "IMGUI_BACKEND_DATA", bd);
#elif defined(__APPLE__)
    viewport->PlatformHandleRaw = (void*)glfwGetCocoaWindow(vd->Window);
#endif
	glfwSetWindowPos(vd->Window, static_cast<int>(viewport->Pos.x),
...
{
	auto vd = static_cast<ImGui_ImplGlfw_ViewportData*>(viewport->
		PlatformUserData);
...
static void ImGui_ImplGlfw_SetWindowPos(ImGuiViewport* viewport, ImVec2 pos)
{
	...
	glfwSetWindowPos(vd->Window, static_cast<int>(pos.x),
...
static void ImGui_ImplGlfw_SetWindowSize(ImGuiViewport* viewport, ImVec2 size)
{
	...
	glfwSetWindowSize(vd->Window, static_cast<int>(size.x),
...
static ImVec2 ImGui_ImplGlfw_GetWindowFramebufferScale(ImGuiViewport* viewport)
{
	...
	ImGui_ImplGlfw_GetWindowSizeAndFramebufferScale(
			vd->Window, nullptr, &framebuffer_scale);
...
static LRESULT CALLBACK ImGui_ImplGlfw_WndProc(HWND hWnd, UINT msg,
											   WPARAM wParam, LPARAM lParam)
{
	auto bd = static_cast<ImGui_ImplGlfw_Data*>(::GetPropA(
			hWnd, "IMGUI_BACKEND_DATA"));
	ImGuiIO& io = ImGui::GetIO(bd->Context);

	WNDPROC prev_wndproc = bd->PrevWndProc;
	auto viewport = static_cast<ImGuiViewport*>(::GetPropA(
			hWnd, "IMGUI_VIEWPORT"));
	if (viewport != nullptr)
		if (auto vd = static_cast<ImGui_ImplGlfw_ViewportData*>(viewport->
			PlatformUserData))
