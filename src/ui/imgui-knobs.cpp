...
namespace detail
{
void draw_arc(ImVec2 center, float radius, float start_angle, float end_angle,
			  float thickness, ImColor color)
{
...
struct knob
{
		 ...
		 float _angle_max)
	{
		...
		if (flags & ImGuiKnobFlags_Logarithmic)
		{
			...
			t = (ImLog(ImAbs(v)) - ImLog(ImAbs(v_min))) / (
		...
		bool drag_vertical =
				...
				(flags & ImGuiKnobFlags_DragVertical || ImAbs(
						 io.MouseDelta[ImGuiAxis_Y]) > ImAbs(
						 io.MouseDelta[ImGuiAxis_X]));
		...
		angle_min = _angle_min < 0 ? std::numbers::pi_v<float> * 0.75f
						: _angle_min;
		angle_max = _angle_max < 0 ? std::numbers::pi_v<float> * 2.25f
						: _angle_max;
				  ...
				  bool filled, int segments)
	{
		...
		ImGui::GetWindowDrawList()->AddCircleFilled(
				...
				is_active ? color.active
					: (is_hovered ? color.hovered : color.base),
				segments);
	}
	...
	void draw_arc(float radius, float size, float start_angle, float end_angle,
				  color_set color)
	{
		auto track_radius = radius * this->radius;
		auto track_size = size * this->radius * 0.5f + 0.0001f;

		detail::draw_arc(center, track_radius, start_angle, end_angle,
						 track_size,
						 is_active ? color.active
							 : (is_hovered ? color.hovered : color.base));
	}
		...
		float angle_max)
{
	if (flags & ImGuiKnobFlags_Logarithmic && v_min <= 0.0 && v_max >= 0.0)
	{
		...
		v_max = ImMax(v_min, v_max);
		// this ensures that in the cornercase v_max is still at least ge v_min
		*p_value = ImMax(ImMin(*p_value, v_max), v_min);
		// this ensures that in the cornercase p_value is within the range
	}
	...
	// Draw title
	// Piotr Piwoni - Modified this to ignore provided labels custom internal
	// ID if it was provided.
	if (!(flags & ImGuiKnobFlags_NoTitle))
	{
		// Find '##' in the label.
		const char* displayLabel = label;
		const char* poundPos = strstr(label, "##");
		if (poundPos)
			displayLabel = label;

		const int length = poundPos ? static_cast<int>(poundPos - label)
							   : static_cast<int>(strlen(label));

		ImVec2 titleSize = ImGui::CalcTextSize(displayLabel,
											   displayLabel + length,
											   true,
											   width);
		...
		ImGui::SetCursorPosX(ImGui::GetCursorPosX() +
							 (width - titleSize[0]) * 0.5f);

		ImGui::TextUnformatted(displayLabel, displayLabel + length);
	}

	// Draw knob
	knob<DataType> k(label, data_type, p_value, v_min, v_max, speed,
					 width * 0.5f, format, flags, angle_min, angle_max);
	...
	if (!(flags & ImGuiKnobFlags_NoInput))
	{
		...
		auto changed = ImGui::DragScalar("###knob_drag", data_type, p_value,
...
color_set GetPrimaryColorSet()
{
	...
	return {colors[ImGuiCol_ButtonActive],
		...
		float angle_max)
{
	...
	case ImGuiKnobVariant_Tick:
	{
		...
		knob.draw_tick(0.5f, 0.85f, 0.08f, knob.angle,
	...
	{
		knob.draw_circle(0.85f, detail::GetSecondaryColorSet(), true, 32);
		knob.draw_dot(0.12f, 0.6f, knob.angle, detail::GetPrimaryColorSet(),
					  true, 12);
		break;
	}
	...
	case ImGuiKnobVariant_Wiper:
	{
		...
		knob.draw_arc(0.8f, 0.41f, knob.angle_min, knob.angle_max,
					  detail::GetTrackColorSet());

		if (knob.t > 0.01f)
		{
			knob.draw_arc(0.8f, 0.43f, knob.angle_min, knob.angle,
						  detail::GetPrimaryColorSet());
		}
		break;
	}
	...
	case ImGuiKnobVariant_WiperDot:
	{
		...
		knob.draw_arc(0.85f, 0.41f, knob.angle_min, knob.angle_max,
					  detail::GetTrackColorSet());
		knob.draw_dot(0.1f, 0.85f, knob.angle, detail::GetPrimaryColorSet(),
					  true, 12);
		break;
	}
	case ImGuiKnobVariant_Stepped:
	{
		for (auto n = 0.f; n < steps; n++)
		{
			...
			knob.draw_tick(0.7f, 0.9f, 0.04f, angle,
						   detail::GetPrimaryColorSet());
		}

		knob.draw_circle(0.6f, detail::GetSecondaryColorSet(), true, 32);
		knob.draw_dot(0.12f, 0.4f, knob.angle, detail::GetPrimaryColorSet(),
					  true, 12);
		break;
	}
	case ImGuiKnobVariant_Space:
	{
		knob.draw_circle(0.3f - knob.t * 0.1f, detail::GetSecondaryColorSet(),
						 true, 16);

		if (knob.t > 0.01f)
		{
			knob.draw_arc(0.4f, 0.15f, knob.angle_min - 1.0f, knob.angle - 1.0f,
						  detail::GetPrimaryColorSet());
			knob.draw_arc(0.6f, 0.15f, knob.angle_min + 1.0f, knob.angle + 1.0f,
						  detail::GetPrimaryColorSet());
			knob.draw_arc(0.8f, 0.15f, knob.angle_min + 3.0f, knob.angle + 3.0f,
						  detail::GetPrimaryColorSet());
		}
		break;
	}
